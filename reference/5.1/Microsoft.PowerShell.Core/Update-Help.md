...
> [!IMPORTANT]
> `Update-Help` erfordert Administratorrechte.
>
...
> Ein Beispiel für die Camel-Case-Schreibweise lautet: `Start-Process powershell.exe -Verb RunAs`.
>
> Sie können Hilfedateien auch aktualisieren, indem Sie das Menü Element Windows PowerShell-Hilfe aktualisieren im Menü Hilfe in Windows PowerShell Integrated Scripting Environment (ISE) verwenden.
>
> Das Windows PowerShell-Hilfe Element aktualisieren führt ein `Update-Help` Cmdlet ohne Parameter aus.
> Um die Hilfe für Module im Verzeichnis zu aktualisieren `$PSHOME` , starten Sie Windows PowerShell ISE, indem Sie die Option als Administrator ausführen verwenden.
...
### Beispiel 4: Automatisches Aktualisieren von Hilfedateien

In diesem Beispiel wird ein geplanter Auftrag erstellt, mit dem die Hilfe für alle Module täglich um 3:00 Uhr aktualisiert wird.
...
Mit dem- `Register-ScheduledJob` Cmdlet wird ein geplanter Auftrag erstellt, der einen- `Update-Help` Befehl ausführt. Der Befehl verwendet den **Credential** -Parameter, um `Update-Help` mithilfe der Anmelde Informationen eines Mitglieds der Gruppe "Administratoren" auf dem Computer auszuführen. Der Wert des-Parameter **Auslösers** ist ein `New-JobTrigger` Befehl, der einen Auftrags--Befehl erstellt, der den Auftrag täglich um 3:00 Uhr startet.

`Register-ScheduledJob`Starten Sie PowerShell mit der Option **als Administrator ausführen** , um den Befehl auszuführen. PowerShell fordert Sie zur Eingabe des Kennworts für den Benutzer auf, der im **Credential** -Parameter angegeben ist. Die Anmelde Informationen werden mit dem geplanten Auftrag gespeichert. Sie werden nicht aufgefordert, wenn der Auftrag ausgeführt wird.

Sie können das `Get-ScheduledJob` Cmdlet verwenden, um den geplanten Auftrag anzuzeigen, das `Set-ScheduledJob` Cmdlet verwenden, um es zu ändern, und das `Unregister-ScheduledJob` Cmdlet verwenden, um es zu löschen. Sie können den geplanten Auftrag auch unter folgendem Pfad in der Aufgabenplanung anzeigen und verwalten:

`Task Scheduler Library\Microsoft\Windows\PowerShell\ScheduledJobs`.

### Beispiel 5: Aktualisieren von Hilfedateien auf mehreren Computern aus einer Dateifreigabe
...
### Beispiel 6: erhalten einer Liste mit aktualisierten Hilfedateien
...
### Beispiel 7: Suchen von Modulen, die aktualisierbare Hilfe unterstützen
...
### Beispiel 8: Inventur aktualisierte Hilfedateien
