...
Erfordert, dass `Hyper-V` (Version `1.1` oder weniger) installiert ist.

```powershell
#Requires -Modules @{ ModuleName="Hyper-V"; MaximumVersion="1.1" }
```

Erfordert, dass eine beliebige Version von `PSScheduledJob` und `PSWorkflow` installiert ist.

```powershell
#Requires -Modules PSWorkflow, PSScheduledJob
```

Wenn Sie den `RequiredVersion` Schlüssel verwenden, stellen Sie sicher, dass die Versions Zeichenfolge genau mit der gewünschten Versions Zeichenfolge übereinstimmt.
