...
Beim Debuggen wird ein Skript überprüft, während es ausgeführt wird, um Fehler in den Skript Anweisungen zu identifizieren und zu korrigieren. Der PowerShell-Debugger unterstützt Sie bei der Untersuchung und Identifizierung von Fehlern und Ineffizienzen in Ihren Skripts, Funktionen, Befehlen, PowerShell-Workflows, PowerShell DSC-Konfigurationen (DSC) oder Ausdrücken.
...
Der PowerShell-Debugger kann auch zum Debuggen von PowerShell-Workflows in der PowerShell-Konsole oder in Windows PowerShell ISE verwendet werden. Ab PowerShell 5,0 können Sie in laufenden Aufträgen oder Prozessen Debuggen, entweder lokal oder Remote.

Sie können die Funktionen des PowerShell-Debuggers verwenden, um ein PowerShell-Skript, eine Funktion, einen Befehl, einen Workflow oder einen Ausdruck während der Ausführung zu überprüfen. Der PowerShell-Debugger enthält eine Reihe von Cmdlets, mit denen Sie Breakpoints festlegen, Breakpoints verwalten und die-aufrufsstapel anzeigen können.
...
Wenn Sie einen Breakpoint erreichen, geben Sie die Debugger-Umgebung ein. Die Eingabeaufforderung ändert sich, sodass Sie mit "[dbg]:" beginnt. Wenn Sie einen Workflow Debuggen, ist die Eingabeaufforderung "[WF dbg]". Sie können die Eingabeaufforderung anpassen.
...
- Sie können Workflow Variablen anzeigen, während Sie im Debugger sind, aber das Festlegen von Workflow Variablen aus dem Debugger wird nicht unterstützt.
- Die Vervollständigung mit der Tab-Taste im Workflow Debugger ist nicht verfügbar.
- Workflow Debugging funktioniert nur mit synchroner Ausführung von Workflows aus einem PowerShell-Skript. Sie können Workflows nicht debuggen, wenn Sie als Auftrag ausgeführt werden (mit dem **AsJob** -Parameter).
- Andere Szenarios mit gehosttem Debuggen, z. b. ein Workflow, der einen anderen Workflow oder einen Workflow zum Aufrufen eines Skripts aufrufen, werden

Im folgenden Beispiel wird das Debuggen eines Workflows veranschaulicht. Wenn der Debugger in die Workflow Funktion wechselt, wechselt die Debugger-Eingabeaufforderung zu "[wfdbg]".
