...
### Beispiel 4: Konvertieren der SDDL für Active Directory Zugriffsrechte in ein pscustomobject
...
Der erste Befehl verwendet Active Directory Service Interfaces (ADSI), um das Benutzerobjekt zu erhalten, und speichert es in der Variablen.

Der zweite Befehl verwendet das `ConvertFrom-SddlString` Cmdlet, um die Textdarstellung der SDDL-Zeichenfolge zu erhalten, die in der SDDL-Eigenschaft des Objekts enthalten ist, das die Sicherheits Beschreibung darstellt.

Er verwendet den- `-Type` Parameter, um anzugeben, dass die SDDL-Zeichenfolge einen Active Directory Sicherheits Deskriptor darstellt.
