...
Verwenden Sie einen **Import-PSSession** - Befehl zum Importieren von Befehlen aus einer benutzerdefinierten Shell (z. B. einer Microsoft Exchange Server-Shell) oder aus einer Sitzung, die Windows PowerShell-Module und -Snap-Ins bzw. andere nicht in der aktuellen Sitzung enthaltene Elemente enthält.
...
Durch diesen Parameter können Sie den Befehl in Windows PowerShell als Hintergrundauftrag ausführen.
Weitere Informationen finden Sie unter %%amp;quot;about_Jobs%%amp;quot;.

Bei Verwendung von **Import-PSSession** fügt Windows PowerShell die importierten Befehlen einem temporären Modul hinzu, das nur in Ihrer Sitzung vorhanden ist, und gibt ein Objekt zurück, von dem das Modul dargestellt wird.
...
Das **Import-PSSession** -Cmdlet verwendet die implizite Remotingfunktion von Windows PowerShell.
...
In der Regel würde das Modul allen Sitzungen über einen **Import-Module** -Befehl in einem Windows PowerShell-Profil hinzugefügt werden. Profile werden in PSSessions jedoch nicht ausgeführt.
...
Da Funktionen Vorrang vor Cmdlets haben, führt Windows PowerShell die importierte **Get-Date** -Funktion aus, durch die ein Julianisches Datum zurückgegeben wird.
...
Der vierte Befehl ruft den Namen des Windows PowerShell-Snap-Ins ab, durch das das ursprüngliche **Get-Date** -Cmdlet der aktuellen Sitzung hinzugefügt wurde.
...
Wenn Sie ein Modulobjekt senden, verwendet Windows PowerShell jedoch die **ToString** -Methode für das Modulobjekt, das den Modulnamen zurückgibt.
...
- Anwendung:
Alle Dateien außer Windows PowerShell-Dateien in den Pfaden, die in der Path-Umgebungsvariablen ($env:path) der Remotesitzung aufgelistet sind (z. B. TXT-, EXE- und DLL-Dateien).
- Cmdlet.
...
### -Modul
Gibt und ein Array von Befehlen in den Windows PowerShell-Snap-Ins und-Modulen an.
...
* **Import-PSSession** basiert auf der Windows PowerShell-Remoting-Infrastruktur. Zur Verwendung dieses Cmdlets muss der Computer für das WS-Management-Remoting konfiguriert sein. Weitere Informationen finden Sie unter about_Remote und about_Remote_Requirements.
* **Import-PSSession** importiert keine Variablen oder Windows PowerShell-Anbieter.
...
* Da Windows PowerShell-Profile in PSSessions nicht ausgeführt werden, stehen **Import-PSSession** keine Befehle zur Verfügung, die einer Sitzung durch ein Profil hinzugefügt werden. Zum Importieren von Befehlen aus einem Profil verwenden Sie einen Invoke-Command-Befehl, um das Profil vor dem Befehlsimport in der PSSession manuell auszuführen.
