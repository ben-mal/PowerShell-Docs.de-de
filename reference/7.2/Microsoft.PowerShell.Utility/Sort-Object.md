...
Der **Status** ist eine aufgezählte Eigenschaft. " **Beendet** " hat den Wert " **1** " und "wird **ausgeführt** " den Wert **4**. Der **Absteig** Ende Parameter ist auf festgelegt `$True` , sodass ausgestellte  Prozesse vor beendeten Prozessen angezeigt werden.  **Display Name** legt den **Absteig** enden Parameter auf fest `$False` , um die anzeigen Amen in alphabetischer Reihenfolge zu sortieren.
