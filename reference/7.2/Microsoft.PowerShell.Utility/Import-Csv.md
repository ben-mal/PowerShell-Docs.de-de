...
Ab PowerShell 6,0 `Import-Csv` unterstützt jetzt das erweiterte W3C-Protokolldatei Format.
...
`Import-Csv` unterstützt auch das erweiterte W3C-Protokoll Format. Zeilen, die mit beginnen, `#` werden als Kommentare behandelt und ignoriert, wenn der Kommentar nicht mit beginnt `#Fields:` und eine durch Trennzeichen getrennte Liste von Spaltennamen enthält. In diesem Fall verwendet das Cmdlet diese Spaltennamen. Dies ist das Standardformat für Windows IIS und andere Webserver Protokolle. Weitere Informationen finden Sie unter [erweitertes Protokolldatei Format](https://www.w3.org/TR/WD-logfile.html).
