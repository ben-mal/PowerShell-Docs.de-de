...
- **Pscommandpath** : enthält den vollständigen Pfad und den Dateinamen des Skripts, das den aktuellen Befehl aufgerufen hat. Der Wert dieser Eigenschaft wird nur aufgefüllt, wenn der Aufrufer ein Skript ist.
...
> [!NOTE]
> Diese Variable ist nur verfügbar, wenn die `PSAnsiRendering` experimentelle Funktion aktiviert ist. Weitere Informationen finden Sie unter [about_Experimental_Features](about_Experimental_Features.md) und [Verwenden von experimentellen Features](/powershell/scripting/learn/experimental-features).

Ab PowerShell 7,2 können Sie nun `$PSStyle` auf die automatische Variable zugreifen, um das Rendering der ANSI-Zeichen folgen Ausgabe anzuzeigen und zu ändern. Die-Variable enthält die folgenden Eigenschaften:

- **Reset** : deaktiviert alle Dekorationen
- Im **Hintergrund** vorgesteueretes Objekt zum Steuern der Hintergrund Färbung
- **Blink** blinkt
- **Blinkoff** -blinkt ausschalten
- **Fett** formatiert: fett formatiert
- **Boldoff** -wird fett formatiert
- **Vordergrund** : das Objekt zum Steuern der Vordergrundfarbe.
- **Formatierung** : steuert die Standard Formatierung für Ausgabestreams.
- **Ausgeblendet** : wird ausgeblendet am
- **Hiddenoff** -schaltet ausgeblendet
- **Outputrends** -steuern, wann das Ausgabe Rendering verwendet wird
- **Umgekehrtes** umkehren
- **Reverseoff** -schaltet umkehren
- **Kursiv** -kursiv formatiert
- **Italicoff** -kursiv formatiert
- Unter **Streichung: unter** Streichung wird unterstrichen
- **Underlineoff-unter** Streichung wird deaktiviert

Die Basismember geben Zeichenfolgen von ANSI-Escapesequenzen zurück, die ihren Namen zugeordnet sind.
Die Werte können beliebig angepasst werden. Beispielsweise können Sie Fett in unterstrichen ändern. Die Eigenschaften Namen erleichtern Ihnen das Erstellen von ergänzten Zeichen folgen mithilfe der Vervollständigung mit der Tab-Taste:

```powershell
"$($PSStyle.Background.LightCyan)Power$($PSStyle.Underline)$($PSStyle.Bold)Shell$($PSStyle.Reset)"
```

Das `$PSStyle.Background` -Element und das-Element `$PSStyle.Foreground` sind Zeichen folgen, die die ANSI-Escapesequenzen für die 16 Standard Konsolen Farben und eine- `Rgb()` Methode zum Angeben von 24-Bit-Farben enthalten. Die Werte sind feststellbar und können beliebig viele ANSI-Escapesequenzen enthalten.

`$PSStyle.Formatting` ist ein gespeichertes Objekt zum Steuern der Standard Formatierung von Debug-, Fehler-, verbose-und Warnmeldungen. Sie können auch Attribute wie fett formatieren und unterstreichen steuern. Es ersetzt `$Host.PrivateData` die Methode zum Verwalten von Farben für das Formatieren von Rendering. `$Host.PrivateData` ist weiterhin aus Gründen der Abwärtskompatibilität vorhanden, ist jedoch nicht mit verbunden `$PSStyle.Formatting` .

`$PSStyle.OutputRendering` ist eine Enumeration `System.Management.Automation.OutputRendering` mit den Werten:

- **Automatisch**: Dies ist die Standardeinstellung. Wenn der Host virtualterminal unterstützt, wird ANSI immer unverändert weitergegeben, andernfalls Klartext.
- **ANSI**: ANSI wird immer unverändert durchlaufen
- **Klartext**: ANSI-Escapesequenzen werden immer entfernt, sodass es nur nur-Text ist.
- **Hostonly**: Dies ist das macOS-Verhalten, bei dem die ANSI-Escapesequenzen in der umgeleiteten oder weitergeleiteten Ausgabe entfernt werden.
...
- **Pabdition** : Diese Eigenschaft hat den Wert "Desktop" für PowerShell 4 und niedriger sowie PowerShell 5,1 in voll funktionsfähigen Windows-Editionen. Diese Eigenschaft hat den Wert "Core" für PowerShell 6 und höher sowie PowerShell PowerShell 5,1 auf Editionen mit eingeschränktem Speicherbedarf wie Windows Nano Server oder Windows IOT.
...
- **Platt Form** Plattform, auf der das Betriebssystem ausgeführt wird. Der Wert unter Linux und macOS ist **UNIX**. Prüfen Sie `$IsMacOs` und `$IsLinux`.
- **Pscompatibleversions** : PowerShell-Versionen, die mit der aktuellen Version kompatibel sind
- **Psremotingprotocolversion** : die Version des PowerShell-Remote Verwaltungs Protokolls.
