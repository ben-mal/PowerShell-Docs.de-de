...
### <span data-ttu-id="bddb2-109">Sitzung</span><span class="sxs-lookup"><span data-stu-id="bddb2-109">Session</span></span>
...
### <span data-ttu-id="bddb2-119">ContainerId</span><span class="sxs-lookup"><span data-stu-id="bddb2-119">ContainerId</span></span>
...
## <span data-ttu-id="bddb2-141">BEISPIELE</span><span class="sxs-lookup"><span data-stu-id="bddb2-141">EXAMPLES</span></span>
...
### <span data-ttu-id="bddb2-153">Beispiel 3: Ausführen eines Befehls in einer permanenten Verbindung</span><span class="sxs-lookup"><span data-stu-id="bddb2-153">Example 3: Run a command in a persistent connection</span></span>
...
### <span data-ttu-id="bddb2-207">Beispiel 9: einschließen lokaler Variablen in einem Befehl, der auf einem Remote Computer ausgeführt wird</span><span class="sxs-lookup"><span data-stu-id="bddb2-207">Example 9: Include local variables in a command run on a remote computer</span></span>
...
### <span data-ttu-id="bddb2-311">-Zuweisung Richtung</span><span class="sxs-lookup"><span data-stu-id="bddb2-311">-AllowRedirection</span></span>
...
<span data-ttu-id="bddb2-325">Der WinRM-Dienst wählt mit dem Anwendungsnamen einen Listener für die Verbindungsanforderung aus.</span><span class="sxs-lookup"><span data-stu-id="bddb2-325">The WinRM service uses the application name to select a listener to service the connection request.</span></span>
...
<span data-ttu-id="bddb2-392">Der Standardwert ist der Wert der Einstellungs `$PSSessionConfigurationName` Variablen auf dem lokalen Computer.</span><span class="sxs-lookup"><span data-stu-id="bddb2-392">The default value is the value of the `$PSSessionConfigurationName` preference variable on the local computer.</span></span> <span data-ttu-id="bddb2-393">Wenn diese Einstellungs Variable nicht festgelegt ist, lautet der Standardwert **Microsoft. PowerShell**.</span><span class="sxs-lookup"><span data-stu-id="bddb2-393">If this preference variable isn't set, the default is **Microsoft.PowerShell**.</span></span> <span data-ttu-id="bddb2-394">Weitere Informationen finden Sie unter [about_Preference_Variables](about/about_Preference_Variables.md).</span><span class="sxs-lookup"><span data-stu-id="bddb2-394">For more information, see [about_Preference_Variables](about/about_Preference_Variables.md).</span></span>
...
### <span data-ttu-id="bddb2-395">-ConnectionUri</span><span class="sxs-lookup"><span data-stu-id="bddb2-395">-ConnectionUri</span></span>

<span data-ttu-id="bddb2-396">Gibt einen URI (Uniform Resource Identifier) an, der den Verbindungsendpunkt für die Sitzung definiert.</span><span class="sxs-lookup"><span data-stu-id="bddb2-396">Specifies a Uniform Resource Identifier (URI) that defines the connection endpoint of the session.</span></span>
<span data-ttu-id="bddb2-397">Der URI muss vollqualifiziert sein.</span><span class="sxs-lookup"><span data-stu-id="bddb2-397">The URI must be fully qualified.</span></span>

<span data-ttu-id="bddb2-398">Das Format dieser Zeichenfolge lautet wie folgt:</span><span class="sxs-lookup"><span data-stu-id="bddb2-398">The format of this string is as follows:</span></span>

`<Transport>://<ComputerName>:<Port>/<ApplicationName>`

<span data-ttu-id="bddb2-399">Der Standardwert lautet:</span><span class="sxs-lookup"><span data-stu-id="bddb2-399">The default value is as follows:</span></span>

`http://localhost:5985/WSMAN`

<span data-ttu-id="bddb2-400">Wenn Sie keinen Verbindungs-URI angeben, können Sie die Parameter " **US** " und " **Port** " verwenden, um die Verbindungs-URI-Werte anzugeben.</span><span class="sxs-lookup"><span data-stu-id="bddb2-400">If you don't specify a connection URI, you can use the **UseSSL** and **Port** parameters to specify the connection URI values.</span></span>

<span data-ttu-id="bddb2-401">Gültige Werte für das **Transport** -Segment des URI sind „HTTP“ und „HTTPS“.</span><span class="sxs-lookup"><span data-stu-id="bddb2-401">Valid values for the **Transport** segment of the URI are HTTP and HTTPS.</span></span> <span data-ttu-id="bddb2-402">Wenn Sie einen Verbindungs-URI mit einem Transport Segment angeben, aber keinen Port angeben, wird die Sitzung mit den Standardports erstellt: 80 für http und 443 für HTTPS.</span><span class="sxs-lookup"><span data-stu-id="bddb2-402">If you specify a connection URI with a Transport segment, but don't specify a port, the session is created with the standards ports: 80 for HTTP and 443 for HTTPS.</span></span> <span data-ttu-id="bddb2-403">Wenn Sie die Standardports für PowerShell-Remoting verwenden möchten, geben Sie Port 5985 für http oder 5986 für HTTPS an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-403">To use the default ports for PowerShell remoting, specify port 5985 for HTTP or 5986 for HTTPS.</span></span>

<span data-ttu-id="bddb2-404">Wenn der Zielcomputer die Verbindung an einen anderen URI umleitet, verhindert PowerShell die Umleitung, es sei denn, Sie verwenden den Parameter " **Zuweisung** " im Befehl.</span><span class="sxs-lookup"><span data-stu-id="bddb2-404">If the destination computer redirects the connection to a different URI, PowerShell prevents the redirection unless you use the **AllowRedirection** parameter in the command.</span></span>
...
### <span data-ttu-id="bddb2-405">-Containerid</span><span class="sxs-lookup"><span data-stu-id="bddb2-405">-ContainerId</span></span>

<span data-ttu-id="bddb2-406">Gibt ein Array von Container-IDs an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-406">Specifies an array of container IDs.</span></span>
...
### <span data-ttu-id="bddb2-407">-Credential</span><span class="sxs-lookup"><span data-stu-id="bddb2-407">-Credential</span></span>

<span data-ttu-id="bddb2-408">Gibt ein Benutzerkonto an, das über die Berechtigung zum Ausführen dieser Aktion verfügt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-408">Specifies a user account that has permission to perform this action.</span></span> <span data-ttu-id="bddb2-409">Der Standardwert ist der aktuelle Benutzer.</span><span class="sxs-lookup"><span data-stu-id="bddb2-409">The default is the current user.</span></span>

<span data-ttu-id="bddb2-410">Geben Sie einen Benutzernamen ein, z. b. **USER01** oder **Domain01\User01** , oder geben Sie ein **PSCredential** -Objekt ein, das vom `Get-Credential` Cmdlet generiert wurde.</span><span class="sxs-lookup"><span data-stu-id="bddb2-410">Type a user name, such as **User01** or **Domain01\User01** , or enter a **PSCredential** object generated by the `Get-Credential` cmdlet.</span></span> <span data-ttu-id="bddb2-411">Wenn Sie einen Benutzernamen eingeben, werden Sie zur Eingabe des Kennworts aufgefordert.</span><span class="sxs-lookup"><span data-stu-id="bddb2-411">If you type a user name, you're prompted to enter the password.</span></span>

<span data-ttu-id="bddb2-412">Anmelde Informationen werden in einem [PSCredential](/dotnet/api/system.management.automation.pscredential) -Objekt gespeichert, und das Kennwort wird als [SecureString](/dotnet/api/system.security.securestring)gespeichert.</span><span class="sxs-lookup"><span data-stu-id="bddb2-412">Credentials are stored in a [PSCredential](/dotnet/api/system.management.automation.pscredential) object and the password is stored as a [SecureString](/dotnet/api/system.security.securestring).</span></span>

> [!NOTE]
> <span data-ttu-id="bddb2-413">Weitere Informationen zum Schutz von **SecureString** -Daten finden Sie unter [wie sicher ist SecureString?](/dotnet/api/system.security.securestring#how-secure-is-securestring).</span><span class="sxs-lookup"><span data-stu-id="bddb2-413">For more information about **SecureString** data protection, see [How secure is SecureString?](/dotnet/api/system.security.securestring#how-secure-is-securestring).</span></span>
...
### <span data-ttu-id="bddb2-414">-Enablenetworkaccess</span><span class="sxs-lookup"><span data-stu-id="bddb2-414">-EnableNetworkAccess</span></span>

<span data-ttu-id="bddb2-415">Gibt an, dass mit diesem Cmdlet Loopback Sitzungen ein interaktives Sicherheits Token hinzugefügt wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-415">Indicates that this cmdlet adds an interactive security token to loopback sessions.</span></span> <span data-ttu-id="bddb2-416">Mit dem interaktiven Token können Sie die Befehle in der Loopbacksitzung ausführen, um Daten von anderen Computern abzurufen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-416">The interactive token lets you run commands in the loopback session that get data from other computers.</span></span> <span data-ttu-id="bddb2-417">Beispielsweise können Sie einen Befehl in der Sitzung ausführen, mit dem XML-Dateien von einem Remotecomputer auf den lokalen Computer kopiert werden.</span><span class="sxs-lookup"><span data-stu-id="bddb2-417">For example, you can run a command in the session that copies XML files from a remote computer to the local computer.</span></span>

<span data-ttu-id="bddb2-418">Eine Loopback Sitzung ist eine **PSSession** , die auf demselben computerbasiert und endet.</span><span class="sxs-lookup"><span data-stu-id="bddb2-418">A loopback session is a **PSSession** that originates and ends on the same computer.</span></span> <span data-ttu-id="bddb2-419">Um eine Loopback Sitzung zu erstellen, lassen Sie den **Computername** -Parameter aus, oder legen Sie seinen Wert auf "dot ( `.` )", "localhost" oder den Namen des lokalen Computers fest.</span><span class="sxs-lookup"><span data-stu-id="bddb2-419">To create a loopback session, omit the **ComputerName** parameter or set its value to dot (`.`), localhost, or the name of the local computer.</span></span>

<span data-ttu-id="bddb2-420">Standardmäßig werden Loopback Sitzungen mit einem Netzwerk Token erstellt, das möglicherweise nicht über ausreichende Berechtigungen zum Authentifizieren bei Remote Computern verfügt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-420">By default, loopback sessions are created using a network token, which might not provide sufficient permission to authenticate to remote computers.</span></span>

<span data-ttu-id="bddb2-421">Der **EnableNetworkAccess** -Parameter ist nur bei Loopbacksitzungen wirksam.</span><span class="sxs-lookup"><span data-stu-id="bddb2-421">The **EnableNetworkAccess** parameter is effective only in loopback sessions.</span></span> <span data-ttu-id="bddb2-422">Wenn Sie **enablenetworkaccess** verwenden, wenn Sie eine Sitzung auf einem Remote Computer erstellen, ist der Befehl erfolgreich, aber der-Parameter wird ignoriert.</span><span class="sxs-lookup"><span data-stu-id="bddb2-422">If you use **EnableNetworkAccess** when you create a session on a remote computer, the command succeeds, but the parameter is ignored.</span></span>

<span data-ttu-id="bddb2-423">Sie können den Remote Zugriff in einer Loopback Sitzung mithilfe des Werts " **kredssp** " des Parameters " **Authentication** " zulassen, der die Sitzungs Anmelde Informationen an andere Computer delegiert.</span><span class="sxs-lookup"><span data-stu-id="bddb2-423">You can allow remote access in a loopback session using the **CredSSP** value of the **Authentication** parameter, which delegates the session credentials to other computers.</span></span>

<span data-ttu-id="bddb2-424">Um den Computer vor böswilligem Zugriff zu schützen, können getrennte Loopback Sitzungen mit interaktiven Tokens, die mithilfe von **enablenetworkaccess** erstellt wurden, nur von dem Computer wieder hergestellt werden, auf dem die Sitzung erstellt wurde.</span><span class="sxs-lookup"><span data-stu-id="bddb2-424">To protect the computer from malicious access, disconnected loopback sessions that have interactive tokens, which are those created using **EnableNetworkAccess** , can be reconnected only from the computer on which the session was created.</span></span> <span data-ttu-id="bddb2-425">Die Verbindung von getrennten Sitzungen, die die CredSSP-Authentifizierung verwenden, kann von anderen Computern wiederhergestellt werden.</span><span class="sxs-lookup"><span data-stu-id="bddb2-425">Disconnected sessions that use CredSSP authentication can be reconnected from other computers.</span></span> <span data-ttu-id="bddb2-426">Weitere Informationen finden Sie unter `Disconnect-PSSession`.</span><span class="sxs-lookup"><span data-stu-id="bddb2-426">For more information, see `Disconnect-PSSession`.</span></span>

<span data-ttu-id="bddb2-427">Dieser Parameter wurde in PowerShell 3,0 eingeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-427">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="bddb2-428">-FilePath</span><span class="sxs-lookup"><span data-stu-id="bddb2-428">-FilePath</span></span>

<span data-ttu-id="bddb2-429">Gibt ein lokales Skript an, das dieses Cmdlet auf einem oder mehreren Remote Computern ausführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-429">Specifies a local script that this cmdlet runs on one or more remote computers.</span></span> <span data-ttu-id="bddb2-430">Geben Sie den Pfad und den Dateinamen des Skripts ein, oder übergeben Sie einen Skript Pfad an `Invoke-Command` .</span><span class="sxs-lookup"><span data-stu-id="bddb2-430">Enter the path and filename of the script, or pipe a script path to `Invoke-Command`.</span></span> <span data-ttu-id="bddb2-431">Das Skript muss auf dem lokalen Computer oder in einem Verzeichnis vorhanden sein, auf das der lokale Computer zugreifen kann.</span><span class="sxs-lookup"><span data-stu-id="bddb2-431">The script must exist on the local computer or in a directory that the local computer can access.</span></span> <span data-ttu-id="bddb2-432">Verwenden Sie **argumentlist** , um die Werte der Parameter im Skript anzugeben.</span><span class="sxs-lookup"><span data-stu-id="bddb2-432">Use **ArgumentList** to specify the values of parameters in the script.</span></span>

<span data-ttu-id="bddb2-433">Wenn Sie diesen Parameter verwenden, konvertiert PowerShell den Inhalt der angegebenen Skriptdatei in einen Skriptblock, überträgt den Skriptblock an den Remote Computer und führt ihn auf dem Remote Computer aus.</span><span class="sxs-lookup"><span data-stu-id="bddb2-433">When you use this parameter, PowerShell converts the contents of the specified script file to a script block, transmits the script block to the remote computer, and runs it on the remote computer.</span></span>
...
### <span data-ttu-id="bddb2-434">-Hidecomputername</span><span class="sxs-lookup"><span data-stu-id="bddb2-434">-HideComputerName</span></span>

<span data-ttu-id="bddb2-435">Gibt an, dass dieses Cmdlet den Computernamen der einzelnen Objekte aus der Ausgabe Anzeige auslässt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-435">Indicates that this cmdlet omits the computer name of each object from the output display.</span></span> <span data-ttu-id="bddb2-436">Standardmäßig wird der Name des Computers, der das Objekt generiert, in der Anzeige angezeigt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-436">By default, the name of the computer that generated the object appears in the display.</span></span>

<span data-ttu-id="bddb2-437">Dieser Parameter betrifft nur die Ausgabeanzeige.</span><span class="sxs-lookup"><span data-stu-id="bddb2-437">This parameter affects only the output display.</span></span> <span data-ttu-id="bddb2-438">Das Objekt wird nicht geändert.</span><span class="sxs-lookup"><span data-stu-id="bddb2-438">It doesn't change the object.</span></span>
...
### <span data-ttu-id="bddb2-439">-HostName</span><span class="sxs-lookup"><span data-stu-id="bddb2-439">-HostName</span></span>

<span data-ttu-id="bddb2-440">Gibt ein Array von Computernamen für eine Secure Shell (SSH)-basierte Verbindung an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-440">Specifies an array of computer names for a Secure Shell (SSH) based connection.</span></span> <span data-ttu-id="bddb2-441">Dies ähnelt dem **Computername** -Parameter, mit dem Unterschied, dass die Verbindung mit dem Remote Computer anstelle von Windows WinRM über SSH hergestellt wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-441">This is similar to the **ComputerName** parameter except that the connection to the remote computer is made using SSH rather than Windows WinRM.</span></span>

<span data-ttu-id="bddb2-442">Dieser Parameter wurde in PowerShell 6,0 eingeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-442">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="bddb2-443">-Indisconnectedsession</span><span class="sxs-lookup"><span data-stu-id="bddb2-443">-InDisconnectedSession</span></span>

<span data-ttu-id="bddb2-444">Gibt an, dass dieses Cmdlet einen Befehl oder ein Skript in einer getrennten Sitzung ausführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-444">Indicates that this cmdlet runs a command or script in a disconnected session.</span></span>

<span data-ttu-id="bddb2-445">Wenn Sie den **indisconnectedsession** -Parameter verwenden, wird `Invoke-Command` auf jedem Remote Computer eine persistente Sitzung erstellt, der durch den **ScriptBlock** -Parameter oder den **FilePath** -Parameter angegebene Befehl gestartet und die Verbindung mit der Sitzung getrennt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-445">When you use the **InDisconnectedSession** parameter, `Invoke-Command` creates a persistent session on each remote computer, starts the command specified by the **ScriptBlock** or **FilePath** parameter, and then disconnects from the session.</span></span> <span data-ttu-id="bddb2-446">Die Befehle werden weiterhin in den getrennten Sitzungen ausgeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-446">The commands continue to run in the disconnected sessions.</span></span> <span data-ttu-id="bddb2-447">Mit **indisconnectedsession** können Sie Befehle ausführen, ohne eine Verbindung mit den Remote Sitzungen aufrechtzuerhalten.</span><span class="sxs-lookup"><span data-stu-id="bddb2-447">**InDisconnectedSession** enables you to run commands without maintaining a connection to the remote sessions.</span></span> <span data-ttu-id="bddb2-448">Und da die Sitzung getrennt wird, bevor Ergebnisse zurückgegeben werden, stellt " **indisconnectedsession** " sicher, dass alle Befehls Ergebnisse an die erneut verbundene Sitzung zurückgegeben werden, statt zwischen Sitzungen aufgeteilt zu werden.</span><span class="sxs-lookup"><span data-stu-id="bddb2-448">And, because the session is disconnected before any results are returned, **InDisconnectedSession** makes sure that all command results are returned to the reconnected session, instead of being split between sessions.</span></span>

<span data-ttu-id="bddb2-449">Sie können " **indisconnectedsession** " nicht mit dem **Sitzungs** Parameter oder dem **AsJob** -Parameter verwenden.</span><span class="sxs-lookup"><span data-stu-id="bddb2-449">You can't use **InDisconnectedSession** with the **Session** parameter or the **AsJob** parameter.</span></span>

<span data-ttu-id="bddb2-450">Befehle, die **indisconnectedsession** verwenden, geben ein **PSSession** -Objekt zurück, das die getrennte Sitzung darstellt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-450">Commands that use **InDisconnectedSession** return a **PSSession** object that represents the disconnected session.</span></span> <span data-ttu-id="bddb2-451">Die Befehlsausgabe wird nicht zurückgegeben.</span><span class="sxs-lookup"><span data-stu-id="bddb2-451">They don't return the command output.</span></span> <span data-ttu-id="bddb2-452">Verwenden Sie zum Herstellen einer Verbindung mit der getrennten Sitzung die `Connect-PSSession` `Receive-PSSession` Cmdlets oder.</span><span class="sxs-lookup"><span data-stu-id="bddb2-452">To connect to the disconnected session, use the `Connect-PSSession` or `Receive-PSSession` cmdlets.</span></span> <span data-ttu-id="bddb2-453">Verwenden Sie das-Cmdlet, um die Ergebnisse von Befehlen zu erhalten, die in der Sitzung ausgeführt wurden `Receive-PSSession` .</span><span class="sxs-lookup"><span data-stu-id="bddb2-453">To get the results of commands that ran in the session, use the `Receive-PSSession` cmdlet.</span></span> <span data-ttu-id="bddb2-454">Um Befehle auszuführen, die eine Ausgabe in einer getrennten Sitzung generieren, legen Sie den Wert der **outputbufferingmode** -Sitzungs Option auf **Drop** fest.</span><span class="sxs-lookup"><span data-stu-id="bddb2-454">To run commands that generate output in a disconnected session, set the value of the **OutputBufferingMode** session option to **Drop**.</span></span> <span data-ttu-id="bddb2-455">Wenn Sie eine Verbindung mit der getrennten Sitzung herstellen möchten, legen Sie das Leerlauf Timeout in der Sitzung fest, damit Sie genügend Zeit zum Herstellen einer Verbindung haben, bevor Sie die Sitzung löschen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-455">If you intend to connect to the disconnected session, set the idle time-out in the session so that it provides sufficient time for you to connect before deleting the session.</span></span>

<span data-ttu-id="bddb2-456">Sie können den Ausgabepuffer Modus und das Leerlauf Timeout im **sessionoption** -Parameter oder in der `$PSSessionOption` Preference-Variablen festlegen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-456">You can set the output buffering mode and idle time-out in the **SessionOption** parameter or in the `$PSSessionOption` preference variable.</span></span> <span data-ttu-id="bddb2-457">Weitere Informationen zu Sitzungs Optionen finden Sie unter `New-PSSessionOption` und [about_Preference_Variables](./about/about_preference_variables.md).</span><span class="sxs-lookup"><span data-stu-id="bddb2-457">For more information about session options, see `New-PSSessionOption` and [about_Preference_Variables](./about/about_preference_variables.md).</span></span>

<span data-ttu-id="bddb2-458">Weitere Informationen zum Feature „Getrennte Sitzungen“ finden Sie unter [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md).</span><span class="sxs-lookup"><span data-stu-id="bddb2-458">For more information about the Disconnected Sessions feature, see [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md).</span></span>

<span data-ttu-id="bddb2-459">Dieser Parameter wurde in PowerShell 3,0 eingeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-459">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="bddb2-460">-InputObject</span><span class="sxs-lookup"><span data-stu-id="bddb2-460">-InputObject</span></span>

<span data-ttu-id="bddb2-461">Gibt die Eingabe für den Befehl an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-461">Specifies input to the command.</span></span> <span data-ttu-id="bddb2-462">Geben Sie eine Variable ein, die die Objekte enthält, oder geben Sie einen Befehl oder einen Ausdruck ein, durch den die Objekte abgerufen werden.</span><span class="sxs-lookup"><span data-stu-id="bddb2-462">Enter a variable that contains the objects or type a command or expression that gets the objects.</span></span>

<span data-ttu-id="bddb2-463">Wenn Sie den **Inputobject** -Parameter verwenden, verwenden `$Input` Sie die automatische Variable im Wert des **ScriptBlock** -Parameters, um die Eingabe Objekte darzustellen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-463">When using the **InputObject** parameter, use the `$Input` automatic variable in the value of the **ScriptBlock** parameter to represent the input objects.</span></span>
...
### <span data-ttu-id="bddb2-464">-Jobname</span><span class="sxs-lookup"><span data-stu-id="bddb2-464">-JobName</span></span>

<span data-ttu-id="bddb2-465">Gibt einen Anzeigenamen für den Hintergrundauftrag an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-465">Specifies a friendly name for the background job.</span></span> <span data-ttu-id="bddb2-466">Standardmäßig werden Aufträge benannt `Job<n>` , wobei `<n>` eine Ordinalzahl ist.</span><span class="sxs-lookup"><span data-stu-id="bddb2-466">By default, jobs are named `Job<n>`, where `<n>` is an ordinal number.</span></span>

<span data-ttu-id="bddb2-467">Wenn Sie den **Jobname** -Parameter in einem Befehl verwenden, wird der Befehl als Auftrag ausgeführt, und es wird `Invoke-Command` ein Auftrags Objekt zurückgegeben, auch wenn Sie **AsJob** nicht in den Befehl einschließen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-467">If you use the **JobName** parameter in a command, the command is run as a job, and `Invoke-Command` returns a job object, even if you don't include **AsJob** in the command.</span></span>

<span data-ttu-id="bddb2-468">Weitere Informationen zu PowerShell-Hintergrund Aufträgen finden Sie unter [about_Jobs](./About/about_Jobs.md).</span><span class="sxs-lookup"><span data-stu-id="bddb2-468">For more information about PowerShell background jobs, see [about_Jobs](./About/about_Jobs.md).</span></span>
...
### <span data-ttu-id="bddb2-469">-KeyFilePath</span><span class="sxs-lookup"><span data-stu-id="bddb2-469">-KeyFilePath</span></span>

<span data-ttu-id="bddb2-470">Gibt einen Schlüssel Dateipfad an, der von Secure Shell (SSH) zum Authentifizieren eines Benutzers auf einem Remote Computer verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-470">Specifies a key file path used by Secure Shell (SSH) to authenticate a user on a remote computer.</span></span>

<span data-ttu-id="bddb2-471">SSH ermöglicht die Ausführung der Benutzerauthentifizierung über private und öffentliche Schlüssel als Alternative zur grundlegenden Kenn Wort Authentifizierung.</span><span class="sxs-lookup"><span data-stu-id="bddb2-471">SSH allows user authentication to be performed via private and public keys as an alternative to basic password authentication.</span></span> <span data-ttu-id="bddb2-472">Wenn der Remote Computer für die Schlüssel Authentifizierung konfiguriert ist, kann dieser Parameter verwendet werden, um den Schlüssel zur Identifizierung des Benutzers bereitzustellen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-472">If the remote computer is configured for key authentication, then this parameter can be used to provide the key that identifies the user.</span></span>

<span data-ttu-id="bddb2-473">Dieser Parameter wurde in PowerShell 6,0 eingeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-473">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="bddb2-474">-Nonewscope</span><span class="sxs-lookup"><span data-stu-id="bddb2-474">-NoNewScope</span></span>

<span data-ttu-id="bddb2-475">Gibt an, dass dieses Cmdlet den angegebenen Befehl im aktuellen Bereich ausführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-475">Indicates that this cmdlet runs the specified command in the current scope.</span></span> <span data-ttu-id="bddb2-476">Standardmäßig `Invoke-Command` führt Befehle in Ihrem eigenen Bereich aus.</span><span class="sxs-lookup"><span data-stu-id="bddb2-476">By default, `Invoke-Command` runs commands in their own scope.</span></span>

<span data-ttu-id="bddb2-477">Dieser Parameter ist nur in Befehlen gültig, die in der aktuellen Sitzung ausgeführt werden, d. h. in Befehlen, bei denen beide Parameter, **ComputerName** und **Session** , weglassen wurden.</span><span class="sxs-lookup"><span data-stu-id="bddb2-477">This parameter is valid only in commands that are run in the current session, that is, commands that omit both the **ComputerName** and **Session** parameters.</span></span>

<span data-ttu-id="bddb2-478">Dieser Parameter wurde in PowerShell 3,0 eingeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-478">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="bddb2-479">-Port</span><span class="sxs-lookup"><span data-stu-id="bddb2-479">-Port</span></span>

<span data-ttu-id="bddb2-480">Gibt den Netzwerkport auf dem Remote Computer an, der für diesen Befehl verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-480">Specifies the network port on the remote computer that is used for this command.</span></span> <span data-ttu-id="bddb2-481">Zum Herstellen einer Verbindung mit einem Remotecomputer muss der Remotecomputer den für die Verbindung verwendeten Port abhören.</span><span class="sxs-lookup"><span data-stu-id="bddb2-481">To connect to a remote computer, the remote computer must be listening on the port that the connection uses.</span></span> <span data-ttu-id="bddb2-482">Die Standardports sind 5985 (WinRM-Port für http) und 5986 (WinRM-Port für HTTPS).</span><span class="sxs-lookup"><span data-stu-id="bddb2-482">The default ports are 5985 (WinRM port for HTTP) and 5986 (WinRM port for HTTPS).</span></span>

<span data-ttu-id="bddb2-483">Konfigurieren Sie vor der Verwendung eines alternativen Ports den WinRM-Listener auf dem Remotecomputer, um diesen Port abzuhören.</span><span class="sxs-lookup"><span data-stu-id="bddb2-483">Before using an alternate port, configure the WinRM listener on the remote computer to listen at that port.</span></span> <span data-ttu-id="bddb2-484">Um den Listener zu konfigurieren, geben Sie die folgenden beiden Befehle an der PowerShell-Eingabeaufforderung ein:</span><span class="sxs-lookup"><span data-stu-id="bddb2-484">To configure the listener, type the following two commands at the PowerShell prompt:</span></span>

`Remove-Item -Path WSMan:\Localhost\listener\listener* -Recurse`

`New-Item -Path WSMan:\Localhost\listener -Transport http -Address * -Port \<port-number\>`

<span data-ttu-id="bddb2-485">Verwenden Sie den **Port** -Parameter nur, wenn dies erforderlich ist.</span><span class="sxs-lookup"><span data-stu-id="bddb2-485">Don't use the **Port** parameter unless you must.</span></span> <span data-ttu-id="bddb2-486">Der im Befehl festgelegte Port gilt für alle Computer oder Sitzungen, für die der Befehl ausgeführt wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-486">The port that is set in the command applies to all computers or sessions on which the command runs.</span></span> <span data-ttu-id="bddb2-487">Eine alternative Porteinstellung kann verhindern, dass der Befehl auf allen Computern ausgeführt wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-487">An alternate port setting might prevent the command from running on all computers.</span></span>
...
### <span data-ttu-id="bddb2-488">-Remotedebug</span><span class="sxs-lookup"><span data-stu-id="bddb2-488">-RemoteDebug</span></span>

<span data-ttu-id="bddb2-489">Wird verwendet, um den aufgerufenen Befehl im Debugmodus in der PowerShell-Remote Sitzung auszuführen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-489">Used to run the invoked command in debug mode in the remote PowerShell session.</span></span>
...
### <span data-ttu-id="bddb2-490">-Runasadministrator</span><span class="sxs-lookup"><span data-stu-id="bddb2-490">-RunAsAdministrator</span></span>

<span data-ttu-id="bddb2-491">Gibt an, dass dieses Cmdlet einen Befehl als Administrator aufruft.</span><span class="sxs-lookup"><span data-stu-id="bddb2-491">Indicates that this cmdlet invokes a command as an Administrator.</span></span>
...
### <span data-ttu-id="bddb2-492">-ScriptBlock</span><span class="sxs-lookup"><span data-stu-id="bddb2-492">-ScriptBlock</span></span>

<span data-ttu-id="bddb2-493">Gibt die auszuführenden Befehle an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-493">Specifies the commands to run.</span></span> <span data-ttu-id="bddb2-494">Schließen Sie die Befehle in geschweiften Klammern `{ }` ein, um einen Skriptblock zu erstellen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-494">Enclose the commands in curly braces `{ }` to create a script block.</span></span>
<span data-ttu-id="bddb2-495">Dieser Parameter ist erforderlich.</span><span class="sxs-lookup"><span data-stu-id="bddb2-495">This parameter is required.</span></span>

<span data-ttu-id="bddb2-496">Standardmäßig werden alle Variablen im Befehl auf dem Remotecomputer ausgewertet.</span><span class="sxs-lookup"><span data-stu-id="bddb2-496">By default, any variables in the command are evaluated on the remote computer.</span></span> <span data-ttu-id="bddb2-497">Um lokale Variablen in den Befehl einzuschließen, verwenden Sie **argumentlist**.</span><span class="sxs-lookup"><span data-stu-id="bddb2-497">To include local variables in the command, use **ArgumentList**.</span></span>
...
### <span data-ttu-id="bddb2-498">-Sitzung</span><span class="sxs-lookup"><span data-stu-id="bddb2-498">-Session</span></span>

<span data-ttu-id="bddb2-499">Gibt ein Array von Sitzungen an, in denen dieses Cmdlet den Befehl ausführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-499">Specifies an array of sessions in which this cmdlet runs the command.</span></span> <span data-ttu-id="bddb2-500">Geben Sie eine Variable ein, die **PSSession** -Objekte enthält, oder einen Befehl, der die **PSSession** -Objekte erstellt oder abruft, z `New-PSSession` . b. den `Get-PSSession` Befehl oder.</span><span class="sxs-lookup"><span data-stu-id="bddb2-500">Enter a variable that contains **PSSession** objects or a command that creates or gets the **PSSession** objects, such as a `New-PSSession` or `Get-PSSession` command.</span></span>

<span data-ttu-id="bddb2-501">Wenn Sie eine **PSSession** erstellen, stellt PowerShell eine permanente Verbindung mit dem Remote Computer her.</span><span class="sxs-lookup"><span data-stu-id="bddb2-501">When you create a **PSSession** , PowerShell establishes a persistent connection to the remote computer.</span></span> <span data-ttu-id="bddb2-502">Verwenden Sie eine **PSSession** , um eine Reihe verwandter Befehle auszuführen, die Daten gemeinsam nutzen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-502">Use a **PSSession** to run a series of related commands that share data.</span></span> <span data-ttu-id="bddb2-503">Verwenden Sie den **Computername** -Parameter, um einen einzelnen Befehl oder eine Reihe von nicht verknüpften Befehlen auszuführen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-503">To run a single command or a series of unrelated commands, use the **ComputerName** parameter.</span></span> <span data-ttu-id="bddb2-504">Weitere Informationen finden Sie unter [about_PSSessions](./About/about_PSSessions.md).</span><span class="sxs-lookup"><span data-stu-id="bddb2-504">For more information, see [about_PSSessions](./About/about_PSSessions.md).</span></span>
...
### <span data-ttu-id="bddb2-505">-Sessionname</span><span class="sxs-lookup"><span data-stu-id="bddb2-505">-SessionName</span></span>

<span data-ttu-id="bddb2-506">Gibt einen Anzeigenamen für eine getrennte Sitzung an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-506">Specifies a friendly name for a disconnected session.</span></span> <span data-ttu-id="bddb2-507">Sie können den Namen verwenden, um in nachfolgenden Befehlen auf die Sitzung zu verweisen, z. b. einen- `Get-PSSession` Befehl.</span><span class="sxs-lookup"><span data-stu-id="bddb2-507">You can use the name to refer to the session in subsequent commands, such as a `Get-PSSession` command.</span></span> <span data-ttu-id="bddb2-508">Dieser Parameter ist nur mit dem **InDisconnectedSession** -Parameter gültig.</span><span class="sxs-lookup"><span data-stu-id="bddb2-508">This parameter is valid only with the **InDisconnectedSession** parameter.</span></span>

<span data-ttu-id="bddb2-509">Dieser Parameter wurde in PowerShell 3,0 eingeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-509">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="bddb2-510">-Sessionoption</span><span class="sxs-lookup"><span data-stu-id="bddb2-510">-SessionOption</span></span>

<span data-ttu-id="bddb2-511">Gibt erweiterte Optionen für die Sitzung an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-511">Specifies advanced options for the session.</span></span> <span data-ttu-id="bddb2-512">Geben Sie ein **sessionoption** -Objekt ein, z. b. ein Objekt, das Sie mit dem `New-PSSessionOption` Cmdlet erstellen, oder eine Hash Tabelle, in der die Schlüssel Sitzungs Optionsnamen und die Werte Sitzungs Optionswerte sind.</span><span class="sxs-lookup"><span data-stu-id="bddb2-512">Enter a **SessionOption** object, such as one that you create using the `New-PSSessionOption` cmdlet, or a hash table in which the keys are session option names and the values are session option values.</span></span>

<span data-ttu-id="bddb2-513">Die Standardwerte für die Optionen werden durch den Wert der Einstellungs `$PSSessionOption` Variablen bestimmt, sofern festgelegt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-513">The default values for the options are determined by the value of the `$PSSessionOption` preference variable, if it's set.</span></span> <span data-ttu-id="bddb2-514">Andernfalls werden die Standardwerte durch Optionen festgelegt, die in der Sitzungskonfiguration festgelegt sind.</span><span class="sxs-lookup"><span data-stu-id="bddb2-514">Otherwise, the default values are established by options set in the session configuration.</span></span>

<span data-ttu-id="bddb2-515">Die Sitzungs Optionswerte haben Vorrang vor Standardwerten für Sitzungen, die in der `$PSSessionOption` Preference-Variablen und in der Sitzungs Konfiguration festgelegt sind.</span><span class="sxs-lookup"><span data-stu-id="bddb2-515">The session option values take precedence over default values for sessions set in the `$PSSessionOption` preference variable and in the session configuration.</span></span> <span data-ttu-id="bddb2-516">Allerdings haben Sie Vorrang vor maximalen Werten, Kontingenten oder Grenzwerten, die in der Sitzungs Konfiguration festgelegt sind.</span><span class="sxs-lookup"><span data-stu-id="bddb2-516">However, they don't take precedence over maximum values, quotas, or limits set in the session configuration.</span></span>

<span data-ttu-id="bddb2-517">Eine Beschreibung der Sitzungs Optionen, die die Standardwerte enthalten, finden Sie unter `New-PSSessionOption` .</span><span class="sxs-lookup"><span data-stu-id="bddb2-517">For a description of the session options that includes the default values, see `New-PSSessionOption`.</span></span> <span data-ttu-id="bddb2-518">Weitere Informationen zur Einstellungs `$PSSessionOption` Variablen finden Sie unter [about_Preference_Variables](About/about_Preference_Variables.md).</span><span class="sxs-lookup"><span data-stu-id="bddb2-518">For information about the `$PSSessionOption` preference variable, see [about_Preference_Variables](About/about_Preference_Variables.md).</span></span>
<span data-ttu-id="bddb2-519">Weitere Informationen zu Sitzungs Konfigurationen finden Sie unter [about_Session_Configurations](About/about_Session_Configurations.md).</span><span class="sxs-lookup"><span data-stu-id="bddb2-519">For more information about session configurations, see [about_Session_Configurations](About/about_Session_Configurations.md).</span></span>
...
### <span data-ttu-id="bddb2-520">-SshConnection</span><span class="sxs-lookup"><span data-stu-id="bddb2-520">-SSHConnection</span></span>

<span data-ttu-id="bddb2-521">Dieser Parameter nimmt ein Array von Hash Tabellen an, in denen jede Hash Tabelle einen oder mehrere Verbindungsparameter enthält, die zum Herstellen einer Secure Shell (SSH)-Verbindung erforderlich sind.</span><span class="sxs-lookup"><span data-stu-id="bddb2-521">This parameter takes an array of hash tables where each hash table contains one or more connection parameters needed to establish a Secure Shell (SSH) connection.</span></span> <span data-ttu-id="bddb2-522">Der **SshConnection** -Parameter ist nützlich zum Erstellen mehrerer Sitzungen, in denen jede Sitzung andere Verbindungsinformationen erfordert.</span><span class="sxs-lookup"><span data-stu-id="bddb2-522">The **SSHConnection** parameter is useful for creating multiple sessions where each session requires different connection information.</span></span>

<span data-ttu-id="bddb2-523">Die Hash Tabelle verfügt über die folgenden Member:</span><span class="sxs-lookup"><span data-stu-id="bddb2-523">The hashtable has the following members:</span></span>

- <span data-ttu-id="bddb2-524">**Computername** (oder **Hostname** )</span><span class="sxs-lookup"><span data-stu-id="bddb2-524">**ComputerName** (or **HostName** )</span></span>
- <span data-ttu-id="bddb2-525">**Port**</span><span class="sxs-lookup"><span data-stu-id="bddb2-525">**Port**</span></span>
- <span data-ttu-id="bddb2-526">**UserName**</span><span class="sxs-lookup"><span data-stu-id="bddb2-526">**UserName**</span></span>
- <span data-ttu-id="bddb2-527">**KeyFilePath** (oder **identityfilepath** )</span><span class="sxs-lookup"><span data-stu-id="bddb2-527">**KeyFilePath** (or **IdentityFilePath** )</span></span>

<span data-ttu-id="bddb2-528">**Computername** (oder **Hostname** ) ist das einzige erforderliche Schlüssel-Wert-Paar.</span><span class="sxs-lookup"><span data-stu-id="bddb2-528">**ComputerName** (or **HostName** ) is the only key-value pair that is required.</span></span>

<span data-ttu-id="bddb2-529">Dieser Parameter wurde in PowerShell 6,0 eingeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-529">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="bddb2-530">-Sshtransport</span><span class="sxs-lookup"><span data-stu-id="bddb2-530">-SSHTransport</span></span>

<span data-ttu-id="bddb2-531">Gibt an, dass die Remote Verbindung mithilfe von Secure Shell (SSH) hergestellt wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-531">Indicates that the remote connection is established using Secure Shell (SSH).</span></span>

<span data-ttu-id="bddb2-532">Standardmäßig verwendet PowerShell Windows WinRM, um eine Verbindung mit einem Remote Computer herzustellen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-532">By default PowerShell uses Windows WinRM to connect to a remote computer.</span></span> <span data-ttu-id="bddb2-533">Dieser Switch erzwingt, dass PowerShell den **Hostname** -Parameter zum Einrichten einer SSH-basierten Remote Verbindung verwendet.</span><span class="sxs-lookup"><span data-stu-id="bddb2-533">This switch forces PowerShell to use the **HostName** parameter for establishing an SSH based remote connection.</span></span>

<span data-ttu-id="bddb2-534">Dieser Parameter wurde in PowerShell 6,0 eingeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-534">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="bddb2-535">-ThrottleLimit</span><span class="sxs-lookup"><span data-stu-id="bddb2-535">-ThrottleLimit</span></span>

<span data-ttu-id="bddb2-536">Gibt die maximale Anzahl von gleichzeitigen Verbindungen an, die zum Ausführen dieses Befehls hergestellt werden können.</span><span class="sxs-lookup"><span data-stu-id="bddb2-536">Specifies the maximum number of concurrent connections that can be established to run this command.</span></span>
<span data-ttu-id="bddb2-537">Wenn Sie diesen Parameter weglassen oder den Wert %%amp;quot;0%%amp;quot; eingeben, wird der Standardwert %%amp;quot;32%%amp;quot; verwendet.</span><span class="sxs-lookup"><span data-stu-id="bddb2-537">If you omit this parameter or enter a value of 0, the default value, 32, is used.</span></span>

<span data-ttu-id="bddb2-538">Die Drosselungsgrenze gilt nur für den aktuellen Befehl und nicht für die Sitzung oder den Computer.</span><span class="sxs-lookup"><span data-stu-id="bddb2-538">The throttle limit applies only to the current command, not to the session or to the computer.</span></span>
...
### <span data-ttu-id="bddb2-539">-UserName</span><span class="sxs-lookup"><span data-stu-id="bddb2-539">-UserName</span></span>

<span data-ttu-id="bddb2-540">Gibt den Benutzernamen für das Konto an, das zum Ausführen eines Befehls auf dem Remote Computer verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-540">Specifies the username for the account used to run a command on the remote computer.</span></span> <span data-ttu-id="bddb2-541">Die Benutzer Authentifizierungsmethode hängt davon ab, wie Secure Shell (SSH) auf dem Remote Computer konfiguriert ist.</span><span class="sxs-lookup"><span data-stu-id="bddb2-541">The user authentication method depends on how Secure Shell (SSH) is configured on the remote computer.</span></span>

<span data-ttu-id="bddb2-542">Wenn SSH für die einfache Kenn Wort Authentifizierung konfiguriert ist, werden Sie zur Eingabe des Benutzer Kennworts aufgefordert.</span><span class="sxs-lookup"><span data-stu-id="bddb2-542">If SSH is configured for basic password authentication, then you'll be prompted for the user password.</span></span>

<span data-ttu-id="bddb2-543">Wenn SSH für die Schlüssel basierte Benutzerauthentifizierung konfiguriert ist, kann ein Schlüssel Dateipfad über den Schlüssel **FilePath** -Parameter bereitgestellt werden, und es erfolgt keine Kenn Wort Eingabeaufforderung.</span><span class="sxs-lookup"><span data-stu-id="bddb2-543">If SSH is configured for key-based user authentication then a key file path can be provided via the **KeyFilePath** parameter and no password prompt occurs.</span></span> <span data-ttu-id="bddb2-544">Wenn sich die Client Benutzer Schlüsseldatei an einem bekannten SSH-Speicherort befindet, ist der **keyFilePath** -Parameter für die Schlüssel basierte Authentifizierung nicht erforderlich, und die Benutzerauthentifizierung erfolgt automatisch basierend auf dem Benutzernamen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-544">If the client user key file is located in an SSH known location, then the **KeyFilePath** parameter isn't needed for key-based authentication, and user authentication occurs automatically based on the username.</span></span> <span data-ttu-id="bddb2-545">Weitere Informationen finden Sie in der SSH-Dokumentation Ihrer Plattform zur Schlüssel basierten Benutzerauthentifizierung.</span><span class="sxs-lookup"><span data-stu-id="bddb2-545">For more information, see your platform's SSH documentation about key-based user authentication.</span></span>

<span data-ttu-id="bddb2-546">Dies ist kein erforderlicher Parameter.</span><span class="sxs-lookup"><span data-stu-id="bddb2-546">This isn't a required parameter.</span></span> <span data-ttu-id="bddb2-547">Wenn der **username** -Parameter nicht angegeben wird, wird der aktuell angemeldete Benutzername für die Verbindung verwendet.</span><span class="sxs-lookup"><span data-stu-id="bddb2-547">If the **UserName** parameter isn't specified, then the current logged on username is used for the connection.</span></span>

<span data-ttu-id="bddb2-548">Dieser Parameter wurde in PowerShell 6,0 eingeführt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-548">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="bddb2-549">-US-</span><span class="sxs-lookup"><span data-stu-id="bddb2-549">-UseSSL</span></span>

<span data-ttu-id="bddb2-550">Gibt an, dass dieses Cmdlet das Secure Sockets Layer (SSL)-Protokoll verwendet, um eine Verbindung mit dem Remote Computer herzustellen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-550">Indicates that this cmdlet uses the Secure Sockets Layer (SSL) protocol to establish a connection to the remote computer.</span></span> <span data-ttu-id="bddb2-551">Standardmäßig wird SSL nicht verwendet.</span><span class="sxs-lookup"><span data-stu-id="bddb2-551">By default, SSL isn't used.</span></span>

<span data-ttu-id="bddb2-552">WS-Management verschlüsselt alle über das Netzwerk übertragenen PowerShell-Inhalte.</span><span class="sxs-lookup"><span data-stu-id="bddb2-552">WS-Management encrypts all PowerShell content transmitted over the network.</span></span> <span data-ttu-id="bddb2-553">Der **Parameter "** " ist ein zusätzlicher Schutz, der die Daten über eine HTTPS-anstelle von http sendet.</span><span class="sxs-lookup"><span data-stu-id="bddb2-553">The **UseSSL** parameter is an additional protection that sends the data across an HTTPS, instead of HTTP.</span></span>

<span data-ttu-id="bddb2-554">Wenn Sie diesen Parameter verwenden, aber SSL auf dem Port, der für den Befehl verwendet wird, nicht verfügbar ist, schlägt der Befehl fehl.</span><span class="sxs-lookup"><span data-stu-id="bddb2-554">If you use this parameter, but SSL isn't available on the port that's used for the command, the command fails.</span></span>
...
### <span data-ttu-id="bddb2-555">-VMID</span><span class="sxs-lookup"><span data-stu-id="bddb2-555">-VMId</span></span>

<span data-ttu-id="bddb2-556">Gibt ein Array von IDs von virtuellen Computern an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-556">Specifies an array of IDs of virtual machines.</span></span>
...
### <span data-ttu-id="bddb2-557">-VMName</span><span class="sxs-lookup"><span data-stu-id="bddb2-557">-VMName</span></span>

<span data-ttu-id="bddb2-558">Gibt ein Array von Namen von virtuellen Computern an.</span><span class="sxs-lookup"><span data-stu-id="bddb2-558">Specifies an array of names of virtual machines.</span></span>
...
### <span data-ttu-id="bddb2-559">-Subsystem</span><span class="sxs-lookup"><span data-stu-id="bddb2-559">-Subsystem</span></span>

<span data-ttu-id="bddb2-560">Gibt das SSH-Subsystem an, das für die neue **PSSession** verwendet wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-560">Specifies the SSH subsystem used for the new **PSSession**.</span></span>

<span data-ttu-id="bddb2-561">Gibt das Subsystem an, das auf dem Ziel verwendet werden soll, wie in sshd_config definiert.</span><span class="sxs-lookup"><span data-stu-id="bddb2-561">This specifies the subsystem to use on the target as defined in sshd_config.</span></span>
<span data-ttu-id="bddb2-562">Das Subsystem startet eine bestimmte Version von PowerShell mit vordefinierten Parametern.</span><span class="sxs-lookup"><span data-stu-id="bddb2-562">The subsystem starts a specific version of PowerShell with predefined parameters.</span></span>
<span data-ttu-id="bddb2-563">Wenn das angegebene Subsystem auf dem Remote Computer nicht vorhanden ist, schlägt der Befehl fehl.</span><span class="sxs-lookup"><span data-stu-id="bddb2-563">If the specified subsystem does not exist on the remote computer, the command fails.</span></span>

<span data-ttu-id="bddb2-564">Wenn dieser Parameter nicht verwendet wird, ist der Standardwert das "PowerShell"-Subsystem.</span><span class="sxs-lookup"><span data-stu-id="bddb2-564">If this parameter is not used, the default is the 'powershell' subsystem.</span></span>
...
### <span data-ttu-id="bddb2-565">CommonParameters</span><span class="sxs-lookup"><span data-stu-id="bddb2-565">CommonParameters</span></span>

<span data-ttu-id="bddb2-566">Dieses Cmdlet unterstützt diese gängigen Parameter: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction und -WarningVariable.</span><span class="sxs-lookup"><span data-stu-id="bddb2-566">This cmdlet supports the common parameters: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction, and -WarningVariable.</span></span> <span data-ttu-id="bddb2-567">Weitere Informationen findest du unter [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216).</span><span class="sxs-lookup"><span data-stu-id="bddb2-567">For more information, see [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216).</span></span>

## <span data-ttu-id="bddb2-568">EINGABEN</span><span class="sxs-lookup"><span data-stu-id="bddb2-568">INPUTS</span></span>

### <span data-ttu-id="bddb2-569">System. Management. Automation. ScriptBlock</span><span class="sxs-lookup"><span data-stu-id="bddb2-569">System.Management.Automation.ScriptBlock</span></span>

<span data-ttu-id="bddb2-570">Sie können einen Befehl in einem Skriptblock an die Pipeline übergeben `Invoke-Command` .</span><span class="sxs-lookup"><span data-stu-id="bddb2-570">You can pipe a command in a script block to `Invoke-Command`.</span></span> <span data-ttu-id="bddb2-571">Verwenden `$Input` Sie die automatische Variable, um die Eingabe Objekte im Befehl darzustellen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-571">Use the `$Input` automatic variable to represent the input objects in the command.</span></span>

## <span data-ttu-id="bddb2-572">AUSGABEN</span><span class="sxs-lookup"><span data-stu-id="bddb2-572">OUTPUTS</span></span>

### <span data-ttu-id="bddb2-573">System. Management. Automation. psremotingjob, System. Management. Automation. Runspaces. PSSession oder die Ausgabe des aufgerufenen Befehls</span><span class="sxs-lookup"><span data-stu-id="bddb2-573">System.Management.Automation.PSRemotingJob, System.Management.Automation.Runspaces.PSSession, or the output of the invoked command</span></span>

<span data-ttu-id="bddb2-574">Dieses Cmdlet gibt ein Auftrags Objekt zurück, wenn Sie den **AsJob** -Parameter verwenden.</span><span class="sxs-lookup"><span data-stu-id="bddb2-574">This cmdlet returns a job object, if you use the **AsJob** parameter.</span></span> <span data-ttu-id="bddb2-575">Wenn Sie den **indisconnectedsession** -Parameter angeben, `Invoke-Command` gibt ein **PSSession** -Objekt zurück.</span><span class="sxs-lookup"><span data-stu-id="bddb2-575">If you specify the **InDisconnectedSession** parameter, `Invoke-Command` returns a **PSSession** object.</span></span> <span data-ttu-id="bddb2-576">Andernfalls wird die Ausgabe des aufgerufenen Befehls zurückgegeben, bei dem es sich um den Wert des **ScriptBlock** -Parameters handelt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-576">Otherwise, it returns the output of the invoked command, which is the value of the **ScriptBlock** parameter.</span></span>

## <span data-ttu-id="bddb2-577">HINWEISE</span><span class="sxs-lookup"><span data-stu-id="bddb2-577">NOTES</span></span>

<span data-ttu-id="bddb2-578">Wenn unter Windows Vista und höheren Versionen des Windows-Betriebssystems der **Computername** -Parameter von `Invoke-Command` zum Ausführen eines Befehls auf dem lokalen Computer verwendet werden soll, müssen Sie PowerShell mithilfe der Option **als Administrator ausführen** ausführen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-578">On Windows Vista, and later versions of the Windows operating system, to use the **ComputerName** parameter of `Invoke-Command` to run a command on the local computer, you must run PowerShell using the **Run as administrator** option.</span></span>

<span data-ttu-id="bddb2-579">Wenn Sie Befehle auf mehreren Computern ausführen, stellt PowerShell eine Verbindung mit den Computern in der Reihenfolge her, in der Sie in der Liste angezeigt werden.</span><span class="sxs-lookup"><span data-stu-id="bddb2-579">When you run commands on multiple computers, PowerShell connects to the computers in the order in which they appear in the list.</span></span> <span data-ttu-id="bddb2-580">Allerdings wird die Befehlsausgabe in der Reihenfolge angezeigt, in der Sie von den Remote Computern empfangen wird, die sich möglicherweise unterscheiden.</span><span class="sxs-lookup"><span data-stu-id="bddb2-580">However, the command output is displayed in the order that it's received from the remote computers, which might be different.</span></span>

<span data-ttu-id="bddb2-581">Fehler, die sich aus dem Befehl ergeben, der `Invoke-Command` ausführt, sind in den Befehls Ergebnissen enthalten.</span><span class="sxs-lookup"><span data-stu-id="bddb2-581">Errors that result from the command that `Invoke-Command` runs are included in the command results.</span></span>
<span data-ttu-id="bddb2-582">Fehler, die in einem lokalen Befehl Fehler mit Abbruch sein würden, werden in einem Remotebefehl als Fehler ohne Abbruch behandelt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-582">Errors that would be terminating errors in a local command are treated as non-terminating errors in a remote command.</span></span> <span data-ttu-id="bddb2-583">Durch diese Strategie wird sichergestellt, dass beim Beenden von Fehlern auf einem Computer der Befehl auf allen Computern, auf denen er ausgeführt wird, nicht geschlossen wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-583">This strategy makes sure that terminating errors on one computer don't close the command on all computers on which it's run.</span></span> <span data-ttu-id="bddb2-584">Diese Vorgehensweise wird selbst dann verwendet, wenn ein Remotebefehl auf einem einzelnen Computer ausgeführt wird.</span><span class="sxs-lookup"><span data-stu-id="bddb2-584">This practice is used even when a remote command is run on a single computer.</span></span>

<span data-ttu-id="bddb2-585">Wenn sich der Remote Computer nicht in einer Domäne befindet, der der lokale Computer vertraut, ist der Computer möglicherweise nicht in der Lage, die Anmelde Informationen des Benutzers zu authentifizieren.</span><span class="sxs-lookup"><span data-stu-id="bddb2-585">If the remote computer isn't in a domain that the local computer trusts, the computer might not be able to authenticate the user's credentials.</span></span> <span data-ttu-id="bddb2-586">Um den Remote Computer der Liste der vertrauenswürdigen Hosts in der WS-Verwaltung hinzuzufügen, verwenden Sie den folgenden Befehl im `WSMAN` Anbieter, wobei `<Remote-Computer-Name>` der Name des Remote Computers ist:</span><span class="sxs-lookup"><span data-stu-id="bddb2-586">To add the remote computer to the list of trusted hosts in WS-Management, use the following command in the `WSMAN` provider, where `<Remote-Computer-Name>` is the name of the remote computer:</span></span>

`Set-Item -Path WSMan:\Localhost\Client\TrustedHosts -Value \<Remote-Computer-Name\>`

<span data-ttu-id="bddb2-587">Wenn Sie eine **PSSession** mit dem **indisconnectedsession** -Parameter trennen, wird der Sitzungs Status **getrennt** , und die Verfügbarkeit ist **None**.</span><span class="sxs-lookup"><span data-stu-id="bddb2-587">When you disconnect a **PSSession** using the **InDisconnectedSession** parameter, the session state is **Disconnected** and the availability is **None**.</span></span> <span data-ttu-id="bddb2-588">Der Wert der **State** Eigenschaft bezieht sich auf die aktuelle Sitzung.</span><span class="sxs-lookup"><span data-stu-id="bddb2-588">The value of the **State** property is relative to the current session.</span></span> <span data-ttu-id="bddb2-589">Der Wert " **getrennt** " bedeutet, dass die **PSSession** nicht mit der aktuellen Sitzung verbunden ist.</span><span class="sxs-lookup"><span data-stu-id="bddb2-589">A value of **Disconnected** means that the **PSSession** isn't connected to the current session.</span></span> <span data-ttu-id="bddb2-590">Dies bedeutet jedoch nicht, dass die **PSSession** von allen Sitzungen getrennt ist.</span><span class="sxs-lookup"><span data-stu-id="bddb2-590">However, it doesn't mean that the **PSSession** is disconnected from all sessions.</span></span> <span data-ttu-id="bddb2-591">Sie kann mit einer anderen Sitzung verbunden sein.</span><span class="sxs-lookup"><span data-stu-id="bddb2-591">It might be connected to a different session.</span></span> <span data-ttu-id="bddb2-592">Um festzustellen, ob Sie eine Sitzungsverbindung herstellen bzw. wiederherstellen können, verwenden Sie die **Availability** -Eigenschaft.</span><span class="sxs-lookup"><span data-stu-id="bddb2-592">To determine whether you can connect or reconnect to the session, use the **Availability** property.</span></span>

<span data-ttu-id="bddb2-593">Ein **Availability** -Wert von **None** gibt an, dass eine Verbindung mit der Sitzung hergestellt werden kann.</span><span class="sxs-lookup"><span data-stu-id="bddb2-593">An **Availability** value of **None** indicates that you can connect to the session.</span></span> <span data-ttu-id="bddb2-594">Der Wert **ausgelastet** gibt an, dass keine Verbindung mit der **PSSession** hergestellt werden kann, da Sie mit einer anderen Sitzung verbunden ist.</span><span class="sxs-lookup"><span data-stu-id="bddb2-594">A value of **Busy** indicates that you can't connect to the **PSSession** because it's connected to another session.</span></span> <span data-ttu-id="bddb2-595">Weitere Informationen zu den Werten der **State** -Eigenschaft von Sitzungen finden Sie unter [runspacestate](/dotnet/api/system.management.automation.runspaces.runspacestate).</span><span class="sxs-lookup"><span data-stu-id="bddb2-595">For more information about the values of the **State** property of sessions, see [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate).</span></span> <span data-ttu-id="bddb2-596">Weitere Informationen zu den Werten der **Availability** -Eigenschaft von Sitzungen finden Sie unter [runspaceavailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability).</span><span class="sxs-lookup"><span data-stu-id="bddb2-596">For more information about the values of the **Availability** property of sessions, see [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability).</span></span>

<span data-ttu-id="bddb2-597">Die Parameter " **Hostname** " und " **SshConnection** " wurden ab PowerShell 6,0 eingeschlossen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-597">The **HostName** and **SSHConnection** parameters were included starting with PowerShell 6.0.</span></span> <span data-ttu-id="bddb2-598">Sie wurden hinzugefügt, um PowerShell-Remoting basierend auf Secure Shell (SSH) bereitzustellen.</span><span class="sxs-lookup"><span data-stu-id="bddb2-598">They were added to provide PowerShell remoting based on Secure Shell (SSH).</span></span> <span data-ttu-id="bddb2-599">PowerShell und SSH werden auf mehreren Plattformen (Windows, Linux, macOS) unterstützt, und PowerShell-Remoting funktioniert auf diesen Plattformen, auf denen PowerShell und SSH installiert und konfiguriert sind.</span><span class="sxs-lookup"><span data-stu-id="bddb2-599">PowerShell and SSH are supported on multiple platforms (Windows, Linux, macOS) and PowerShell remoting works over these platforms where PowerShell and SSH are installed and configured.</span></span> <span data-ttu-id="bddb2-600">Dies ist unabhängig vom vorherigen Windows-Remoting, das auf WinRM basiert, und viele der WinRM-spezifischen Features und Einschränkungen gelten nicht.</span><span class="sxs-lookup"><span data-stu-id="bddb2-600">This is separate from the previous Windows only remoting that is based on WinRM and many of the WinRM specific features and limitations don't apply.</span></span> <span data-ttu-id="bddb2-601">Beispielsweise werden auf WinRM basierende Kontingente, Sitzungs Optionen, benutzerdefinierte Endpunkt Konfiguration und Features zum Trennen und Wiederherstellen von Verbindungen derzeit nicht unterstützt.</span><span class="sxs-lookup"><span data-stu-id="bddb2-601">For example WinRM based quotas, session options, custom endpoint configuration, and disconnect/reconnect features are currently not supported.</span></span> <span data-ttu-id="bddb2-602">Weitere Informationen zum Einrichten von PowerShell-SSH-Remoting finden Sie unter [PowerShell-Remoting über SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span><span class="sxs-lookup"><span data-stu-id="bddb2-602">For more information about how to set up PowerShell SSH remoting, see [PowerShell Remoting Over SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span></span>

## <span data-ttu-id="bddb2-603">VERWANDTE LINKS</span><span class="sxs-lookup"><span data-stu-id="bddb2-603">RELATED LINKS</span></span>

[<span data-ttu-id="bddb2-604">about_PSSessions</span><span class="sxs-lookup"><span data-stu-id="bddb2-604">about_PSSessions</span></span>](./About/about_PSSessions.md)

[<span data-ttu-id="bddb2-605">about_Remote</span><span class="sxs-lookup"><span data-stu-id="bddb2-605">about_Remote</span></span>](./About/about_Remote.md)

[<span data-ttu-id="bddb2-606">about_Remote_Disconnected_Sessions</span><span class="sxs-lookup"><span data-stu-id="bddb2-606">about_Remote_Disconnected_Sessions</span></span>](./About/about_Remote_Disconnected_Sessions.md)

[<span data-ttu-id="bddb2-607">about_Remote_Troubleshooting</span><span class="sxs-lookup"><span data-stu-id="bddb2-607">about_Remote_Troubleshooting</span></span>](./About/about_remote_troubleshooting.md)

[<span data-ttu-id="bddb2-608">about_Remote_Variables</span><span class="sxs-lookup"><span data-stu-id="bddb2-608">about_Remote_Variables</span></span>](./About/about_Remote_Variables.md)

[<span data-ttu-id="bddb2-609">about_Scopes</span><span class="sxs-lookup"><span data-stu-id="bddb2-609">about_Scopes</span></span>](./About/about_scopes.md)

[<span data-ttu-id="bddb2-610">Enter-PSSession</span><span class="sxs-lookup"><span data-stu-id="bddb2-610">Enter-PSSession</span></span>](Enter-PSSession.md)

[<span data-ttu-id="bddb2-611">Exit-PSSession</span><span class="sxs-lookup"><span data-stu-id="bddb2-611">Exit-PSSession</span></span>](Exit-PSSession.md)

[<span data-ttu-id="bddb2-612">Get-PSSession</span><span class="sxs-lookup"><span data-stu-id="bddb2-612">Get-PSSession</span></span>](Get-PSSession.md)

[<span data-ttu-id="bddb2-613">Invoke-Item</span><span class="sxs-lookup"><span data-stu-id="bddb2-613">Invoke-Item</span></span>](../Microsoft.PowerShell.Management/Invoke-Item.md)

[<span data-ttu-id="bddb2-614">New-PSSession</span><span class="sxs-lookup"><span data-stu-id="bddb2-614">New-PSSession</span></span>](New-PSSession.md)

[<span data-ttu-id="bddb2-615">Remove-PSSession</span><span class="sxs-lookup"><span data-stu-id="bddb2-615">Remove-PSSession</span></span>](Remove-PSSession.md)

[<span data-ttu-id="bddb2-616">WSMan-Anbieter</span><span class="sxs-lookup"><span data-stu-id="bddb2-616">WSMan Provider</span></span>](../Microsoft.WsMan.Management/About/about_WSMan_Provider.md)
