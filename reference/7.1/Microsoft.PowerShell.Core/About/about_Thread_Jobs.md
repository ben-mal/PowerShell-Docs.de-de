...
Es gibt zwei Möglichkeiten, einen Thread Auftrag zu starten.

Die erste Möglichkeit ist die Verwendung des- `Start-ThreadJob` Cmdlets. Dieses Cmdlet ist im **Thread Job** -Modul verfügbar, das im Lieferumfang von PowerShell enthalten ist. `Start-ThreadJob` Gibt ein einzelnes Auftrags Objekt zurück, das den laufenden Befehl oder das Skript kapselt und mit allen PowerShell-Cmdlets für die Auftragsbearbeitung verwendet werden kann.

Die zweite Möglichkeit ist mit dem- `ForEach-Object` Cmdlet, mit dem `-Parallel` Parameter Skriptblock Argument zusammen mit dem `-AsJob` Parameter Switch. Dieses Cmdlet gibt ein einzelnes (übergeordnetes) Auftrags Objekt zurück, das einen untergeordneten Auftrag für jede Eingabe enthält, die an das Cmdlet weitergeleitet wird. Jeder untergeordnete Auftrag führt das Skript in einem separaten Thread mit einem ( `-ThrottleLimit` )-Limit aus, um die Anzahl der untergeordneten Aufträge zu einem bestimmten Zeitpunkt auszuführen.
...
|`ForEach-Object` |Startet Thread Aufträge für jedes weitergeleitete Eingabe Objekt, wenn   |
|                 |wird mit-parallel-und-AsJob-Parametern verwendet.             |
